...
def check_samplesheet(FileIn):
    ...
    HEADER = ['patient_id', 'state', 'sample_id', 'fastq_1', 'fastq_2']
    fin = open(FileIn,'r')
    ...
    while True:
        ...
        if line:
            ...
            patient, state, sample, fastQFiles = lspl[0], lspl[1], lspl[2], lspl[3:]
            ##if sample:
            if sample.find(' ') != -1:
            ...
            for fastq in fastQFiles:
                ...
                if fastq[-9:] != '.fastq.gz' and fastq[-6:] != '.fq.gz':
                    print_error("FastQ file", fastq, " does not have extension '.fastq.gz' or '.fq.gz'!",line)
